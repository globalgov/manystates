...
pkgs <- c("WDI", "ggplot2", "dplyr", "countrycode", "manydata", "manystates")
...
GDP <- dplyr::as_tibble(GDP) %>%
  manydata::transmutate(COW_Nr = as.character(countrycode::countrycode(GDP$iso2c,
                                                       origin = "iso2c",
                                                       destination = "cown",
                                                       warn = FALSE)))
```
...
in a way that is consistent with the manyverse with the `import_cshapes()`
function.
