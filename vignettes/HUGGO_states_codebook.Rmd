---
title: "HUGGO states codebook"
output: rmarkdown::html_vignette
author: James Hollway
date: "2025-09-19"
vignette: >
  %\VignetteIndexEntry{HUGGO states codebook}
...
This document provides a brief overview of the coding rationale for key variables in the list of episodes of independent states and state-like entities in the international system provided in `manystates::states$HUGGO`. 
...
The basis is coded as how the state began. We use the categories offered in the ISD dataset:

- _Consolidation_: state created over space where none previously existed 
- _Declaration_: state born from a declaration of independence from a larger state that continues to exist
- _Decolonisation_: state born from decolonisation
- _Dissolution_: state born as a fragment of a larger state that broke apart and ceased to exist (e.g. Austro-Hungarian Empire)
- _Restoration_: state restored after a period of non-existence
- _Secession_: state secedes from larger state that continues to exist
- _Unification_: state born from the voluntary merging of several (typically equally sized) states
...
- _Legislature Informed_: ratification requires that the legislature is informed
- _Unicameral Majority_: ratification requires legislative approval from one legislative chamber
- _Supermajority/Bicameral_: ratification requires legislative approval from two chambers or a supermajority
