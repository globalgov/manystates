...
The `regimes` database contains data on political regimes in the Polity5 and Freedom House datasets. We will focus on Freedom House data.
Freedom House and VDEM projects have slightly different approaches to the
measurement of democracy, which we will explore below.
Let's import the data first.
Due to the size of VDEM data, we rely on the `import_vdem()` function to download the data, contrary to Freedom House dataset which is stored internally. 
...
fh <- dplyr::select(fh, c(Label, cowID, Year, PR, CL, B, C, D)) %>%
  dplyr::rename(StateName = Label, stateID = cowID) %>% 
  # select only the variables of interest
...
Let's compare the 10 states with the highest accountability scores (on average) in each dataset:
...
fh %>% # saving this object to use later
  group_by(StateName, regime_type) %>% # group by country and regime
  count() %>% # count grouped observations
  group_by(StateName) %>% # re-group by country name only 
  mutate(Duplicated = n()) %>% # add duplicated columns for country names
...
There appears to be no consensus across datasets...
...
Let's work with only VDEM data since it has more country year observations.
