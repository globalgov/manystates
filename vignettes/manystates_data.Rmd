...
fh <- dplyr::select(fh, c(StateName, stateID, Year, PR, CL, B, C, D)) %>%
  ...
  dplyr::mutate(regime_type = ifelse(CL > 52, "Liberal Democracy",
                                     "Electoral Democracy")) %>%
  # create a new regime type variable and rename the categories within, benchmark score derived from Scores table in Freedom House Codebook
  drop_na() # drop rows where there is at least one NA obs...
```
...
Let's work with only VDEM data since it has more state year observations.
