...
test_that("missing observations are reported correctly", {
  expect_false(any(grepl("^n/a$", regimes[["Polity5d"]])))
  expect_false(any(grepl("^N/A$", regimes[["Polity5d"]])))
  expect_false(any(grepl("^\\s$", regimes[["Polity5d"]])))
...
test_that("a column indicating an ID source exists", {
  expect_true(any(grepl("ID", colnames(regimes[["Polity5d"]]))))
})
...
#     if (!is.null(regimes[["Polity5d"]]$Beg)) {
#   expect_true(regimes[["Polity5d"]]$Beg[50] < regimes[["Polity5d"]]$Beg[75])
