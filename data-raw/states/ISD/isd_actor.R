...
isd_actor <- utils::read.csv("data-raw/states/ISD/ISD_Version1_Dissemination.csv",
                      stringsAsFactors = F)[,1:7] %>% dplyr::mutate(
                        StatID = dplyr::coalesce(countrycode::countrycode(State.Name, "country.name", "iso3c"),
                                          COW.ID),
                        StatID = dplyrrecode(StatID, "ZAN" = "EAZ"),
                        End = dplyrrecode(as.character(lubridate::dmy(End)), "2011-12-31" = "9999-12-31"),
                        Micro = (Micro == "X")
                      ) %>% manydata::transmutate(
                        Label = gsub("&","and", dplyr::na_if(na_if(State.Name, ""), "State Name")),
                        CowID = COW.ID,
                        Beg = as.character(lubridate::dmy(Start)),
                        New = (New.State == "X")
                      ) %>% dplyr::filter(!is.na(Label)) %>%
  dplyr::select(-c(COW.Nr., New)) %>%
  dplyr::select(StatID, Label, Beg, End, CowID, everything()) %>%
  dplyr::arrange(Beg)
...
# library(sticky)
isd_actor <- dplyr::as_tibble(isd_actor)
# library(haven)
isd_actor <- isd_actor %>% dplyr::mutate(Beg = haven::labelled(Beg, label = "This is a date"))
isd_not <- isd_actor %>% dplyr::filter(Beg > "1990-01-01")

isd_sans <- dplyr::subset(isd_actor, Beg > "1990-01-01")
str(isd_sans)

isd_all <- dplyr::full_join(isd_actor, isd_not)
str(isd_all)

# library(table1)
table1::label(isd_actor$End) <- "This is also a date"
table1::table1(Beg ~ End, isd_actor)
table1::has.label(isd_actor$End)

attr(isd_actor$Beg, 'label') <- "This is a date"

# library(pointblank)
pointblank::scan_data(as_tibble(isd_actor), c("overview", "variables", "missing", "sample"))
pointblank::info_tabular(isd_actor)
