...
countryRegex <- dplyr::tribble(
  ...
  "COD","Democratic Republic of the Congo","dem.*congo|congo(.+)?dem|d[[:punct:]]r[[:punct:]]c[[:punct:]]|belgian.?congo|congo.?free.?state|kinshasa|zaire|zaïre|l.opoldville|drc| droc |rdc|droc|congo-kinshasa|congo dr|d.r. of the congo|congo.+kin",
...
england|united.?kingdom|britain|british(?!.*hondur| east africa)|\buk\b|\bu[[:punct:]]k[[:punct:]]\b|royaume-uni|bermuda|anguilla|cayman|channel.is|akrotiri|dhekelia|falkland.is|virgin.is|gibraltar|guernsey|guernesey|jersey|isle of man|northern.ireland|pitcairn|helena|norfolk|montserrat|south.sandwich.is|caicos|grande.bretagne|england",
  ...
  "GNB","Guinea-Bissau","bissau|(?=.*portu).*guinea|guin[eé]e.bissa[uo]",
  ...
  "KOR","Republic of Korea","(?<!d.p.r. |dpr |democrat |Democratic People's Republic of |north )korea(?!.*d.p.r)|cor[eé]e.*sud|r[eé]publique.de.cor[eé]e",
  "KOS","Kosovo","kosovo",
  "KOT","Kotah","kotah",
  ...
  "NLD","Netherlands","(?!.*ant)(?!.*carib).*netherlands|netherlands.antil|dutch.antil|aruba|curaçao|curacao|pays-bas|bonaire|holland",
  "NOR","Norway","norway|norweg|norv[eè]ge|bouvet|svalbard",
  "NPL","Nepal","n[eé]pal",
  "NRU","Nauru","nauru",
  ...
  "PRK","Democratic People's Republic of Korea","dprk|d.p.r.k|korea.+(d.p.r|dpr|north|dem.*peo.*rep.*)|(d.p.r|dpr|dem.*peo.*rep.*).+korea|north korea|cor[eé]e.*nord",
  ...
  "STP","Sao Tome and Principe","s[aã]o.tom[eé]|sao tome-et-principe|saotome",
  "SUL","Sulu","sulu",
  ...
  "YPR","Yemen People’s Republic","(?=.*peo).*yemen|(?!.*rep)(?=.*dem).*yemen|(?=.*south).*yemen|(?=.*aden).*yemen|(?=.*\bp\\.?d\\.?r).*yemen|aden",
